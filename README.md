...
| 本地规则复盘 | 不调用 Gemini / 智谱，基于搜狐与东方财富公开接口计算盈亏、均线、RSI、量能、板块、F10 公司资料与组合风险 |
| 周策略执行单 | 每周第一个交易日记录持仓成本并生成条件策略；周内每日复盘自动读取、回顾并可因持仓变化升级版本 |
...
3. 东方财富 F10 补充公司资料、主营构成、地区收入、核心概念、板块标签、股东人数、实际控制人和前五大流通股东；
4. 将 F10 业务结构与当日板块、题材、换手率、筹码变化互相印证；
5. 按持仓成本与股数计算市值、浮动盈亏、当日盈亏、权重和规则化压力/支撑；
6. 汇总生成组合摘要、集中度、弱趋势与高换手风险；
7. 每周第一个交易日生成 ISO 周策略执行单，周内每日复盘读取并追加执行回顾。
...
1. 若复盘日是本周第一个交易日，记录当日 `my_stock.txt` 的成本、股数和持有周期，并生成 v1 周策略；
2. 若周内执行时发现还没有策略单，会用当日持仓补建，并在更新记录中注明；
3. 周内每日复盘都会读取同一份周策略，追加“每日执行回顾”；
4. 若关键支撑、压力或突破条件触发，复盘会记录触发原因，默认继续执行原策略；
5. 若持仓成本、股数或标的发生变化，自动生成新版本策略，旧基准保留在更新记录中；
6. 周策略与每日组合摘要会一起写入当天的最终复盘文档。

> 建议在交易日收盘后执行 `provider=local`。估值与板块来自东方财富实时快照，程序会校验快照日期；如果次日补跑历史日期，相关字段会标记为“待核实”，不会把最新快照误写成复盘日数据。F10 的主营构成和股东数据按复盘日截止时间过滤；公司资料、板块标签与核心概念属于当前 F10 快照，复盘文档会明确标注口径。
...
- 周策略 JSON/Markdown：本周持仓成本基准、条件策略、版本更新与每日执行回顾；
