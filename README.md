...
| 本地规则复盘 | 不调用 Gemini / 智谱，基于搜狐与东方财富公开接口计算盈亏、均线、RSI、量能、板块、最新股东人数、融资融券余额与组合风险 |
| 周策略执行单 | 周五收盘后或周末基于日线/周线截图提前生成下一周策略；包含左侧/右侧买入信号、指标搭配与止损线，周内每日自动回顾 |
...
2. 策略单记录当时 `my_stock.txt` 的成本、股数、持有周期、日线/周线指标、左侧买入区、右侧突破触发、买入数量和止损线；
