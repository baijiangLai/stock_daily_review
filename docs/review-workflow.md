...
本地规则引擎适合交易日收盘后执行。个股 OHLC、均线、RSI、量能、当日形态与趋势定义来自搜狐历史日线；估值、板块、财报、公告与新闻来自东方财富公开接口。板块和估值实时快照会校验日期，补跑历史日期时不会把最新数据误写成当日数据。F10 只补充最新股东人数与最新融资融券余额，两项数据均按复盘日截止时间过滤。
...
周策略文件位于 `screenshots/weekly/YYYY-Www/YYYY-Www_交易策略单.md`、同名 JSON 文件与 `charts/` 子目录。周五收盘后或周六/周日运行时，程序会为每只持仓补充日 K 与周 K 截图，使用最近交易日的数据提前生成下一周策略；周末运行不会把周六/周日当成交易日。策略包含持仓成本基准、日线/周线形态与趋势定义、指标、左侧买入区、右侧突破触发、买入数量与止损线；形态使用反包、长影线、十字星、放量/普通阴阳线等规则定义，趋势使用收盘价与 MA5/MA10/MA20 的排列定义。周内每日读取并追加回顾；持仓变化自动生成新版本，止损或关键价位触发则先记录原因，由人工决定是否修改。旧版按年份存放的策略文件会在下一次读取时自动迁移到具体周目录。
